...
import webDevAPI from '../api/api'; // Uncomment when using the API service
...
export default function WebDevServicePopup() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Use the webDevAPI instead of direct fetch
      const result = await webDevAPI.submitLead(formData);
          ...
          <div className="relative z-10">
            ...
            <div className="flex gap-2 mt-4">
              {[1, 2].map(step => (
                <div
                  key={step}
                  className={`flex-1 h-1 rounded-full transition-colors ${step <= currentStep ? 'bg-white' : 'bg-white/30'
                    }`}
                />
              ))}
            </div>
              ...
              <>
                        ...
                        {projectTypes.map(type => {
                          ...
                          return (
                            <button
                              ...
                              className={`p-3 border rounded-lg text-left transition-colors ${formData.projectType === type.value
                                  ? 'border-blue-500 bg-blue-50 text-blue-700'
                                  : 'border-gray-200 hover:border-gray-300 text-gray-700'
                                }`}
                            >
                              <Icon className={`w-5 h-5 mb-1 ${formData.projectType === type.value ? 'text-blue-700' : 'text-gray-600'
                                }`} />
                              <div className={`text-sm font-medium ${formData.projectType === type.value ? 'text-blue-700' : 'text-gray-700'
                                }`}>{type.label}</div>
                            </button>
                          );
                        })}
                ...
                <div className="flex justify-between items-center pt-6 border-t bg-white sticky bottom-0">
                  <button
                    ...
                    className={`px-4 py-2 text-gray-600 ${currentStep === 1 ? 'invisible' : 'hover:text-gray-800'
                      }`}
                  >
                    ← Back
                  </button>
                  ...
                  {currentStep < 2 ? (
                    <button
                      onClick={handleNext}
                      disabled={!canProceed()}
                      className={`px-6 py-2 rounded-lg font-medium flex items-center gap-2 transition-colors ${canProceed()
                    ...
                    <button
                      ...
                      className={`px-8 py-3 rounded-lg font-medium transition-colors ${canProceed() && !isSubmitting
